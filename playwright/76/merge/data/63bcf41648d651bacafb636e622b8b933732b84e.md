...
```
Error: locator.click: Error: strict mode violation: getByRole('button', { name: '参加' }) resolved to 4 elements:
    1) <button disabled type="button" aria-label="グループに参加する" class="chakra-button css-3lb50m">…</button> aka getByRole('button', { name: 'グループに参加する' })
    2) <button type="button" aria-label="今日のランチドラフトに参加する" class="chakra-button css-1x7rxs">…</button> aka getByRole('button', { name: '今日のランチドラフトに参加する' })
    3) <button type="button" aria-label="映画選びドラフトに参加する" class="chakra-button css-1x7rxs">…</button> aka getByRole('button', { name: '映画選びドラフトに参加する' })
    4) <button type="button" aria-label="チームドラフトに参加する" class="chakra-button css-1x7rxs">…</button> aka getByRole('button', { name: 'チームドラフトに参加する' })

Call log:
  - waiting for getByRole('button', { name: '参加' })
...
```ts
  ...
  100 |       
  101 |       // コード入力
  102 |       const codeInput = page.getByLabel('参加コード');
  103 |       await codeInput.fill(invalidCode);
  104 |       
  105 |       // 参加ボタンをクリック
  106 |       const submitButton = page.getByRole('button', { name: '参加' });
  107 |       await submitButton.click();
  108 |       
  109 |       // エラーメッセージが表示されることを確認
  110 |       await draftOps.verifyErrorMessage(errorMessages.invalidCode);
  111 |       
  112 |       // ページ遷移しないことを確認
  ...
  147 |       const recentList = page.locator(selectors.join.recentMeetingsList);
  ...
  157 |       const existingUserList = page.locator(selectors.lobby.userSelect.existingUserList);
...
> 167 |       await submitButton.click();
  ...
  191 |       const urlInput = page.getByLabel('参加URL');
  192 |       
  193 |       // 不正なURL形式
  194 |       await urlInput.fill('not-a-url');
  195 |       const submitButton = page.getByRole('button', { name: '参加' });
  ...
  206 |       const codeInput = page.getByLabel('参加コード');
  207 |       const urlInput = page.getByLabel('参加URL');
  208 |       
  209 |       // コードを入力
